...
and Command =
    ...
    | Clipped of Rectangle * Command
    | Transformed of Matrix3x2 * Command
...
let clip drawing =
    ...
    match commandClip drawing.command with
    | Some r -> r
...
let render target cache drawing = renderCommand target cache drawing.command
